...
const EditClientPage = ({ params }: { params: { id: string } }) => {
  ...
  const handleFolderClick = (item: any) => {
    if (item.type === 'folder' && item.folder) {
      ...
      fetchClientDocuments(params.id, item.id);
    }
  };
  ...
  const handleBackToParent = () => {
    if (folderHistory.length > 0) {
      ...
      fetchClientDocuments(params.id, previousFolderId || null);
    }
  };
  ...
  const handleDeleteDocument = async (docItem: any) => {
    ...
    if (confirm(`Are you sure you want to delete "${fileName}"?`)) {
      try {
        ...
        fetchClientDocuments(params.id, currentFolder?.id || null);
      } catch (error) {
                      ...
                      <div className="text-center py-16 text-gray-500">
                        ...
                        <p className="text-sm">
                          {currentFolder 
                            ...
                            : 'Try uploading some files or creating a new folder'
                          }
                        </p>
                      </div>
                    )}
